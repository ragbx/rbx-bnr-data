...
Relevés au 2026-09-26 (37 IR, 12 267 lignes) :

- **7 `<geogname>` sans `c_id`** : indexés au niveau de l'`archdesc`.
- **55 `<geogname>` vides** (`<geogname source="rue"/>`, sans texte ni
  `coord`), tous dans `FR595126101_MED_LET`. Ils sont déjà vides dans la
  source BnR (`data/ead/bnr/FR595129901_MED_03.xml`) : index « rue » créés
  mais jamais renseignés. Ils sont **conservés** dans le CSV pour
  signalement (filtrer sur `geogname` vide pour les retrouver). La source
  BnR en compte 56 ; le 56e (`<geogname/>` sans `@source`, composant
  `Let_0567`) n'est plus présent après [ead_bnr2mnesys](ead_bnr2mnesys.md).
- **`source`** : `rue` 6 912, vide 4 717, `quartier` 636, `adresse` 1,
  `chrono` 1 (valeur isolée, probable erreur de saisie).
