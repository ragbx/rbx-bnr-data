...
new_ref_az_ko_right = pd.read_csv(
    join("results", "ref", "tmp", f"new_ref_az_ko_right_{new_ref_date}.csv.gz")
)
...
new_ref_az_ko_right_csnodupl = new_ref_az_ko_right[
    ~new_ref_az_ko_right.duplicated(subset="ref_checksum_md5", keep=False)
]
new_ref_az_ko_left_cs_csnodupl = new_ref_az_ko_left_cs[
    ~new_ref_az_ko_left_cs.duplicated(subset="checksum_md5", keep=False)
]
...
new_ref_az_ko_left_cs_csdupl = new_ref_az_ko_left_cs[
    new_ref_az_ko_left_cs.duplicated(subset="checksum_md5", keep=False)
]
...
r = pd.read_csv(join("results", "ref", "tmp", "new_ref_az_ok_20260502.csv.gz"))
r = pd.concat([r, new_ref_az_ok, new_ref_az2_ok])
r.to_csv(
    join("results", "ref", "tmp", "new_ref_az_ok_20260502_it2.csv.gz"), index=False
)

pd.concat([new_ref_az_ko, new_ref_az2_ko]).to_csv(
    join("results", "ref", "tmp", "new_ref_az_ko_left_20260502_it2.csv.gz"), index=False
)
