...
import pandas as pd
from os.path import join, sep, splitext, basename
import csv
...
def get_files2upload(data_file_path, rbx_client, bucket, prefix):
    ...
    for file in data2upload.to_dict(orient='records'):
        file_path = file['path'].replace("/", sep).lstrip(sep)
        if prefix:
...
# Exécution
data_file_name = splitext(basename(input_file))[0]
...
with open(result_file, 'w', newline='') as logfile:
    ...
    files2upload = get_files2upload(input_file, rbx_client, bucket, prefix)
